...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const tag = { engine: 'MUNDO' as const, key: maskKey(matched.sdk_key), pkg, app };

  if (db.server_status.MUNDO.maintenance_mode === 1)
    return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'SERVER_MAINTENANCE' },
      { status: false, code: 'error', message: db.server_status.MUNDO.maintenance_message || 'Server is under maintenance' }, 503, res);
  if (matched.is_blocked)
    return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'INVALID_KEY' },
      { status: false, code: 'error', message: 'SDK key blocked' }, 403, res);

  const exp = new Date(matched.created_at).getTime() + matched.duration_days * 86400 * 1000;
  if (Date.now() > exp)
    return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'EXPIRED_KEY' },
      { status: false, code: 'error', message: 'SDK key expired' }, 403, res);
  ...
  if (!pkgs.has(pkg) && pkgs.size >= matched.pkg_limit)
    return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'PKG_LIMIT_REACHED' },
      { status: false, code: 'error', message: 'Package limit reached' }, 403, res);

  let mb = binds.find((b) => b.pkg_name === pkg && b.app_name === app);
  if (!mb) {
    if (binds.length >= matched.pkg_limit * matched.app_limit)
      return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'APP_LIMIT_REACHED' },
        { status: false, code: 'error', message: 'App limit reached' }, 403, res);
  ...
  if ((mb as any).is_blocked)
    return logged(db, { ...tag, t: new Date().toISOString(), ok: false, code: 'BINDING_BLOCKED' },
      { status: false, code: 'error', message: 'Binding blocked' }, 403, res);

  logVerify(db, { ...tag, t: new Date().toISOString(), ok: true, code: 'VALID' });
  await persistDB(db);
  return enc({
